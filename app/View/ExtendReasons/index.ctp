...
<div class="row">
  <div class="col-sm-7">
    <h1>Administration <small>Extend Reasons</small> <?php echo $this->Html->link('<span class="glyphicon glyphicon-plus"></span>', array('action' => 'add'),array('class' => 'btn btn-primary', 'escape' => false)); ?></h1>
  </div>
  <div class="col-sm-5">
      ...
      <tbody>
        <?php 
          foreach ($reasons as $reason){
            echo "<tr>";
            printf("<td>%s</td>",$reason["Extendreason"]["name"]);
            printf("<td>%s</td>",$reason["Extendreason"]["reason"]);
            echo "<td>";
            echo	$this->Html->link(
								'<span class="glyphicon glyphicon-pencil"></span>',
								array('action' => 'edit', $reason['Extendreason']['id']),
